...
std::unique_ptr<BottomLevelAS> BottomLevelAS::Generate(const std::vector<BottomLevelASMesh>& meshes, ID3D12GraphicsCommandList4* commandList, ID3D12Device5* device)
{
    ...
    blas->m_scratch = GraphicsResource::CreateBufferForRTAccellerationStructure(L"BLAS - scratch", info.ScratchDataSizeInBytes, true, device);
    blas->m_blas = GraphicsResource::CreateBufferForRTAccellerationStructure(L"BLAS - result", info.ResultDataMaxSizeInBytes, false, device);
